...
describe('fandry-checkbox', () => {
  ...
  it('lets slotted content replace the plain-text label', () => {
    ...
    // fandry-popover's own trigger-harness tests). Asserting via the slot's
    // own assignedElements() rather than .label's textContent -- see
    // fandry-radio's equivalent test for why (a jsdom/synthetic-shadow
    // textContent-composition quirk, not a real bug).
    const harness = createElement('slot-harness', { is: SlotHarness });
    document.body.appendChild(harness);

    const checkbox = harness.shadowRoot!.querySelector('fandry-checkbox')!;
