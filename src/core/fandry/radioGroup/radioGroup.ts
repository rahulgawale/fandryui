...
export default class RadioGroup extends Base {
  ...
  private getGroupRadios(): FdRadioElement[] {
    return Array.from(this.querySelectorAll('fandry-radio')).filter(
  ...
  // fandry-radio's <input> lives in its own shadow root: Chrome scopes native
  ...
  updateRadioButtons(selectedValue: string) {
    const radios = this.querySelectorAll('fandry-radio');
