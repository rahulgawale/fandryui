...
describe('fandry-radio-group roving tabindex + arrow-key navigation', () => {
  afterEach(() => {
  ...
  it('gives only the checked radio tabIndex 0, and every other radio -1', async () => {
    ...
    const radios = harness.shadowRoot!.querySelectorAll('fandry-radio');
  ...
  it('ArrowDown moves the roving tabindex, selection, and real focus to the next radio', async () => {
    ...
    // would (same reasoning as fandry-popover's own Escape-key focus test).
    // Spying on the shared HTMLElement.prototype.focus proves fandry-radio's
    ...
    const group = harness.shadowRoot!.querySelector('fandry-radio-group')!;
