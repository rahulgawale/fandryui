...
export default class FdMenuItem extends Base {
  ...
  // A plain field wouldn't tell fandry-menu when a property-bound `disabled`
  ...
  // property mutating in place, so fandry-menu's roving tabindex would
  // otherwise go stale exactly when a row's disabled state flips. This
  // setter dispatches a bubbling `itemchange` fandry-menu listens for (see
  // menu.ts) so the roving tabindex gets recomputed on every real change,
  // without fandry-menu needing to poll or watch each item's properties itself.
  @api
  ...
  // standalone fandry-menu-item (not inside fandry-menu) is still usable on its
  // own -- see fandry-radio's radio.ts for the same default. fandry-menu (when
  // present) reassigns this same prop for roving-tabindex, exactly like
  // fandry-radio-group does to fandry-radio.
  ...
  // (e.g. arrow-key navigation between items) -- see fandry-radio's focus()
