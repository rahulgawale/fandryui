...
export default class FdTooltip extends Base {
  ...
  // close delay: fandry-tooltip's content is meant to be plain descriptive
  ...
  // covers plain text the same way fandry-popover's panel does, and stays
  // rich-content-capable for free (see fandry-popover's own panel slot).
  //
  // Deliberately uncontrolled (no `@api open` the way fandry-popover has) --
  ...
  connectedCallback() {
    ...
    // for on the host itself -- see fandry-popover's connectedCallback for why
    ...
    // fandry-button); this is the same fix, generalized to hover/focus.
  ...
  // `event.target` (not composedPath()) matches fandry-popover's own
  ...
  handleDocumentKeydown = (event: KeyboardEvent) => {
    // Dismiss-only, no focus movement -- unlike fandry-popover's Escape
