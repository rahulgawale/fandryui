...
describe('fandry-menu', () => {
  ...
  it('renders role="menu" with a vertical orientation', async () => {
    ...
    const menu = harness.shadowRoot!.querySelector('fandry-menu')!;
  ...
  it('gives the first enabled item tabIndex 0 and every other item -1 by default', async () => {
    ...
    const items = Array.from(harness.shadowRoot!.querySelectorAll('fandry-menu-item')) as HTMLElement[];
  ...
  it('recomputes roving tabindex when an item\'s disabled property changes in place (not a slotchange)', async () => {
    ...
    const items = Array.from(harness.shadowRoot!.querySelectorAll('fandry-menu-item')) as any[];
    const first = items[0].shadowRoot!.querySelector('.item') as HTMLElement;
    const second = items[1].shadowRoot!.querySelector('.item') as HTMLElement;
  ...
  it('lets a consumer catch a bubbled "select" from an fandry-menu-item via onselect on <fandry-menu>', async () => {
    const harness = createElement('menu-standalone-harness', { is: MenuStandaloneHarness });
    document.body.appendChild(harness);
    await flush();
    ...
    const firstItem = harness.shadowRoot!.querySelector('fandry-menu-item')!;
  ...
  it('does not dispatch "select" when a disabled item is clicked', async () => {
    ...
    const items = harness.shadowRoot!.querySelectorAll('fandry-menu-item');
  ...
  it('composes with fandry-popover: an item selection still bubbles all the way out to the popover host', async () => {
    ...
    const editItem = harness.shadowRoot!.querySelectorAll('fandry-menu-item')[0];
