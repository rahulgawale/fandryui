...
// fandry-menu owns no trigger and no open/close state -- those belong to
// fandry-popover ("a generic overlay primitive with no opinion on what it
...
// which rules out fandry-menu auto-detecting/controlling an ancestor popover.
// Compose the two instead, with the consumer wiring open/close the way any
// fandry-popover consumer already does elsewhere in this codebase:
//
//   <fandry-popover open={menuOpen} ontoggle={handleMenuToggle}>
//     <fandry-button slot="trigger">Actions</fandry-button>
//     <fandry-menu onselect={handleMenuSelect}>
//       <fandry-menu-item value="edit">Edit</fandry-menu-item>
//     </fandry-menu>
//   </fandry-popover>
//
...
//   }
//
// fandry-menu-item's own `select` event already bubbles (see menuItem.ts), so
// fandry-menu doesn't need to listen for or re-dispatch it itself -- a
// consumer's `onselect` on the <fandry-menu> tag catches it natively, the same
// way it would catch a bubbled DOM event from any other descendant.
// fandry-menu's only job is the WAI-ARIA "menu" role/keyboard-navigation
// contract over its slotted fandry-menu-item children.
export default class FdMenu extends Base {
  ...
  // fandry-menu-item dispatches this when one of its own properties (currently
  ...
  // fandry-radio-group's radioGroup.ts for why both renderedCallback *and*
  ...
  private getItems(): FdMenuItemElement[] {
    return Array.from(this.querySelectorAll('fandry-menu-item')) as FdMenuItemElement[];
  }

  // Roving tabindex per the WAI-ARIA menu pattern -- see fandry-radio-group's
  // updateRovingTabIndex for why this can't rely on native per-shadow-root
  // tab stops: every fandry-menu-item's internal element lives in its own
