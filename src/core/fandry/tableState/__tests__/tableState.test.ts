...
describe('FdTableState (extended with a completely custom template)', () => {
  ...
  it('still sorts via the inherited handleHeaderClick, with no fandry-table involved', async () => {
    const element = createElement('custom-table-harness', {
      is: CustomTableHarness
    });
    element.columns = COLUMNS;
  ...
  it('dispatches the same "sortchange" event fandry-table dispatches, unchanged', async () => {
    const element = createElement('custom-table-harness', {
      is: CustomTableHarness
    });
    element.columns = COLUMNS;
