 ...
 * with no template of its own. fandry-table extends this with the library's
 ...
 * and handler here instead of copying fandry-table's internals. See AGENTS.md,
 ...
 * `lwc:is` available for per-row/per-cell customization -- fandry-table's own
 ...
 * it, without fandry-table itself paying the cost of losing Shadow DOM
 * encapsulation.
 *
...
export default class FdTableState extends Base {
   ...
   * `loadingCells` describe a placeholder grid -- fandry-table's own template
   * uses these to render fandry-skeleton rows; a subclass's own template is
   ...
   * -- see fandry-checkbox's checkbox.ts for the general pattern. Defaults
  ...
  private warnIfRowSelectionNeedsRowId() {
    ...
    console.warn(
      'fandry-table: enableRowSelection is combined with a manual* prop (server-driven data) but no getRowId was provided. Row identity defaults to array index, so a selection can silently reattach to a different row once `data` is replaced. Supply getRowId to key selection by stable row identity instead.'
    );
  }
  ...
  private handlePaginationChange = (updater: Updater<PaginationState>) => {
    ...
    // state -- reassigning it here too (the same pattern fandry-checkbox/
    // fandry-input already use for their own @api `checked`/`value`) is what
  ...
  // fandry-table has no column-grouping feature, so tanstack's getHeaderGroups()
  // always resolves to exactly one group -- exposed singular to match, and
  // because a named <slot> (used by fandry-table's own template for the "select
  ...
  /**
   * Renders a column's `cell` as a plain string -- fandry-table's own default
  ...
  /**
   * fandry-table only renders primitives -- richer per-cell markup is tracked
  ...
  protected toDisplayValue(result: unknown, source: string): string {
    ...
    // eslint-disable-next-line no-console
    console.warn(
      `fandry-table: ${source} returned a non-primitive value (${type}), which fandry-table cannot render. See issue #33 for richer cell content support.`
    );
    return '';
  }
  ...
  handleToggleRowSelected(event: Event) {
    ...
    // with either fandry-checkbox or a plain native <input type="checkbox"> a
    // consumer swaps in -- both expose `.checked`, only fandry-checkbox also
  ...
  handleGlobalFilterInput(event: Event) {
    ...
    // target so slot="search" works with fandry-input or a plain native
