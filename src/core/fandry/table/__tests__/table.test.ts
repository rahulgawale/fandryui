...
describe('fandry-table', () => {
  ...
  it('renders fandry-skeleton placeholder rows instead of data while loading', () => {
    const element = createElement('fandry-table', { is: FdTable });
    element.columns = COLUMNS;
    element.data = DATA;
    ...
    expect(loadingRows[0].querySelectorAll('fandry-skeleton').length).toBe(
      COLUMNS.length
    );
  ...
  // fandry-table renders <fandry-checkbox> for selection, not a raw <input> -- its
  // native input lives inside fandry-checkbox's own shadow root, so interacting
  ...
  it('renders a "select all" header checkbox and a per-row checkbox when enableRowSelection is set', () => {
    ...
    expect(
      element.shadowRoot!.querySelectorAll('tbody .selection-cell fandry-checkbox').length
    ).toBe(DATA.length);
  });
  ...
  it('dispatches "rowselectionchange" with the selected row\'s original data when its checkbox is checked', async () => {
    ...
    const firstRowCheckboxHost = element.shadowRoot!.querySelector(
      'tbody .selection-cell fandry-checkbox'
    )!;
  ...
  it('toggling the "select all" checkbox selects and deselects every row', async () => {
    ...
    const rowCheckboxHosts = Array.from(
      element.shadowRoot!.querySelectorAll('tbody .selection-cell fandry-checkbox')
    );
    expect(
  ...
  // that native event (retargeted onto the fandry-input host, `detail: 0` per
  // UIEvent's legacy numeric default) used to reach fandry-table's `oninput`
  // listener a second time and stomp the real typed value back to `0`.
  const typeIntoSearch = (element: Element, value: string) => {
    const searchHost = element.shadowRoot!.querySelector('.table-toolbar fandry-input')!;
  ...
  // fandry-table's slot content only gets distributed via LWC's own compiled
  ...
  it('lets a consumer replace the "select all" checkbox via slot="selection-all", still selecting rows', async () => {
    ...
    // CustomEvent shape only fandry-checkbox happens to provide.
    customSelectAll.checked = true;
    customSelectAll.dispatchEvent(new Event('change', { bubbles: true }));
    await flush();

    const rowCheckboxes = Array.from(
      tableEl.shadowRoot!.querySelectorAll('tbody .selection-cell fandry-checkbox')
  ...
  it('renders a caption when provided and omits it otherwise', () => {
    const withCaption = createElement('fandry-table', { is: FdTable });
    ...
    const withoutCaption = createElement('fandry-table', { is: FdTable });
