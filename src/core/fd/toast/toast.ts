...
export default class FdToast extends Base {
  ...
  // Mirrors fd-alert's own role split: warning/danger are urgent enough to
  ...
  // see fd-tooltip's own handleAnimationEnd for why. Only a completed exit
  ...
  handleAnimationEnd = () => {
    if (this.isClosing) {
      ...
      // itself (already in their own light DOM) to remove this fd-toast
      // from whatever list rendered it -- no need to cross further shadow
      // boundaries. See fd-popover's `toggle` event for the same
      // reasoning.
